...
账号系统启用后，服务启动且数据库没有任何用户时，访问页面会进入“初始化管理员账号”引导页，需创建管理员（用户名 + 密码，密码至少 8 位）。只有管理员能在“成员管理”面板创建、停用、重置密码或删除成员；全部业务接口（含素材图片）都需要登录，未登录接口返回 401，部署脚本使用公开的 `GET /api/health` 做健康检查。
...
1. **界面配置（推荐）**：以管理员登录后，“全部素材 → 标签管理”面板内的“AI 服务配置”填写接口地址、API key 与模型名并保存。key 写入服务端配置文件、页面只显示尾号掩码，提供“测试连接”按钮。普通成员看不到该入口。
...
- 转码产物通过 `/api/media` 以 HTTP Range（206）流式提供，浏览器原生播放。
